...
``aiida-quantum-transport`` comes with a ``.github`` folder that contains continuous integration tests on every commit using `GitHub Actions <https://github.com/features/actions>`_. It will:
...
Once you're happy with your documentation, it's easy to host it online on ReadTheDocs_:
 ...
 #. Import your ``aiida-quantum-transport`` repository (preferably using ``aiida-quantum-transport`` as the project name)

The documentation is now available at `aiida-quantum-transport.readthedocs.io <http://aiida-quantum-transport.readthedocs.io/>`_.
